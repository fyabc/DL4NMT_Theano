...
def main():
    ...
    parser.add_argument('--delib_vocab', action='store', dest='delib_vocab', default=None,
                        help='The vocabulary generated by deliberation model, default is %(default)s')
    ...
    for idx in xrange(args.start, args.end + 1):
        ...
        if not os.path.exists(trans_result_file):
            exec_str = 'python translate_single.py -b 32 {0} -k {1} -p 1 {2} -n {3} {4} {5} {6} {7} {8}\n'.format(
                '-zhen' if zhen else '',
                args.beam_size,
                '' if args.delib_vocab is None else '--delib_vocab {}'.format(args.delib_vocab),
                trans_model_file,
                './data/dic/{}'.format(dic1),
                './data/dic/{}'.format(dic2),
                './data/test/{}'.format(test1),
                trans_result_file,
                './data/dic/{}'.format(dev1) if zhen else '',
            )
