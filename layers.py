...
def param_init_fflayer(options, params, prefix='ff', nin=None, nout=None,
    ...
    params[_p(prefix, 'b')] = np.zeros((nout,)).astype(fX)
...
def param_init_gru(options, params, prefix='gru', nin=None, dim=None):
    ...
    params[_p(prefix, 'b')] = np.zeros((2 * dim,)).astype(fX)
    ...
    params[_p(prefix, 'bx')] = np.zeros((dim,)).astype(fX)
...
def param_init_gru_cond(options, params, prefix='gru_cond',
    ...
    params[_p(prefix, 'Ux')] = Ux
    params[_p(prefix, 'bx')] = np.zeros((dim_nonlin,)).astype(fX)

    U_nl = np.concatenate([ortho_weight(dim_nonlin),
                           ortho_weight(dim_nonlin)], axis=1)
    params[_p(prefix, 'U_nl')] = U_nl
    params[_p(prefix, 'b_nl')] = np.zeros((2 * dim_nonlin,)).astype(fX)

    Ux_nl = ortho_weight(dim_nonlin)
    params[_p(prefix, 'Ux_nl')] = Ux_nl
    params[_p(prefix, 'bx_nl')] = np.zeros((dim_nonlin,)).astype(fX)
    ...
    b_att = np.zeros((dimctx,)).astype(fX)
    params[_p(prefix, 'b_att')] = b_att

    # attention:
    U_att = norm_weight(dimctx, 1)
    params[_p(prefix, 'U_att')] = U_att
    c_att = np.zeros((1,)).astype(fX)
...
def build_model(tparams, O):
    ...
    x_mask = tensor.matrix('x_mask', dtype=fX)
    y = tensor.matrix('y', dtype='int64')
    y_mask = tensor.matrix('y_mask', dtype=fX)
...
def build_sampler(tparams, options, trng, use_noise):
    ...
    init_state = tensor.matrix('init_state', dtype=fX)
